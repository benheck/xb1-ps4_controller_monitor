    ...
    switch(controlSample) {
      ...
      case 2:								//Event Control?
        if (--timer == 0) {				//Did we reach an event point?	
          ...
          if (flightType == 2 and eventTOF[eventCounter]) {  //Has time of flight been enabled and does this event have a flag?
            Serial.print("Event #");
            Serial.print(eventCounter + 1);
            Serial.print(" TOF = ");
            ...
            getTimeDigitalEvent();			//Wait for light transition
          }
          else {
            Serial.print("Event #");
          ...
          eventCounter++;						//Increment counter to next event
...
void showEventCodes() {
	...
	Serial.println("(event number)(space)(event code)(space)(number of ticks to hold event)(optional Time of Flight flag)");
	...
	Serial.println("Example: 3 BBP 250T(press enter) = Press button B, wait for time of flight sample, then wait 2 seconds");
	Serial.println("Example: 4 END(press enter) = Ends event playback.");	
	Serial.println("");
	...
	while(1) {
		...
		switch(parseLine()) {
			...
			case 0:			//Line parsed OK?	
				printSingleEvent(eventCounter);		//Print that line back to user to confirm
...
int parseLine() {					//Parse one line from buffer
  ...
  eventTOFflag = 0 ;        //Clear this flag. If getNumber() finds a T we set flag to make this event trigger a Time of Flight detection
	...
	if (endCheck != 'E') {					//An END command? Then don't parse the last number. Else parse the final number as TIME
		...
		eventTime = getNumber();			//Parse the final number in the line (event time). This also adds the time of flight flag if any
    ...
    eventTOFflag = 0;           //Ensure this is ZERO so time of flight flags will be cleared
  }
  ...
  if (eventTOFflag) {           //Did we find a T at the end of the command?
      eventTOF[eventCounter] = 1;    //Set flag to look for a time of flight after this even triggers
  }  
  else {
      eventTOF[eventCounter] = 0;    //Else ensure it is clear
  }
...
void clearEvents() {			//Clear events (set them all to Event End
	...
	for (int x = 0 ; x < 64 ; x++) {	//Set all events as END
		eventTOF[x] = 0;                //Clear trigger flags
    eventASCII[x] = 0x00454E44;     //END opcode fill
		event[x] = &lastEvent;	
	}
